...
tex \newpage
tex \section{Vision impairment - alignment analysis}
texdoc stlog
tab va2040
texdoc stlog close
...
foreach i in `vlist' { // *forvalues i=1/16 {
   ...
   foreach grpe in 1 2 {
     local aa`i'_`grpe' ""
     eme f_by_`i'_`grpe' , mat(estv)
     local a`i'_`grpe' : di %5.3f `r(r1)'
     local b`i'_`grpe' ""
     forvalues t=1 / `nthres`i'' {
       eme thresholds_`i'_`t'_`grpe', mat(estv)
   ...
   local lab`i': var label `i'
    if $makenewgraphs == 1 {
      ...
      graph export icc_`grp'_`i'v.png , replace width(3000)
      local lab: var label `i'
   }
   tex u`i': `lab' \\
   tex \begin{center}
   tex \includegraphics[width=.8\textwidth]{icc_`grp'_`i'v.png}\\
   tex \end{center}
...
qui foreach uvar in `vlist' {
   foreach grp in 1 2 {
     ...
     nois di in red "eme thresholds_`uvar'_`nthres`uvar''_`grp'"
      eme thresholds_`uvar'_`nthres`uvar''_`grp' , mat(estv)
...
tex \begin{center}
tex \includegraphics[width=1\textwidth]{load_thres_alignment_vision.png}\\
tex \end{center}
